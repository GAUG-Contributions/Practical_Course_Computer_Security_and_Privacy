...
\begin{abstract}
Although the popularity and availability of smartwatches has increased over the past few years, some researches reveal that 
users hold wrong beliefs when it comes to privacy and wearable devices and their privacy concerns vary depending on the context. Hence, raising user awareness about smartwatch data collection, in a clear and understandable way, becomes important. The goal of this work is to propose mechanisms which attract users' attention and provide them information about the data collection of their smartwatches in a clear and understandable way. Our solution implements different types of visual feedback which could be combined with haptic and/or sonar feedback according to the users' preferences. The feedbacks are triggered in real-time whenever a human activity sensor is accessed. Thus, the collection of such data becomes transparent to the users. However, our solution could be adapted for real-life scenarios either if the vendor provides APIs that can indicate sensor usage in a global context instead of an application context or developers of smartwatch applications decide to be transparent about the collected data. While the first option may increase security concerns, the second option may be widely applicable only if it is regulated by laws. 
...
\begin{figure}[t]
\leftline{\includegraphics[width=0.5\textwidth]{img/tizenInfrastructure.png}}
...
This sub section describes the functional and non-functional requirements of the system. Functional requirements describe the functions to be performed by the system or its components whereas a non-functional requirements define the performance attributes of the system or each component.

1) Sensor accesses must be detected and appropriate feedback sent to the user.

2) The user should be able to suspend sensor usage.

3) The user interface should be designed following the general principles and guidelines.
...
\subsection{\textbf{Watch face application}}
